...
export const adminGetAllDeceased = async (req, res) => {
  // const adminMembers = await Member.find({});
  // res.status(StatusCodes.OK).json({ adminMembers });
  const { searchNames, searchCodes, contributionStatus, sort } = req.query;
  const queryObject = {
    // createdBy: req.user.userId,
  };
  ...
  if (contributionStatus && contributionStatus !== 'all') {
    queryObject.contributionStatus = contributionStatus;
  }
  ...
  const adminDeceasedMembers = await DeceasedMember.find(queryObject)
    .sort(sortKey)
    .skip(skip)
    .limit(limit);

  const totalDeceasedMembers = await DeceasedMember.countDocuments(queryObject);
  const numOfPages = Math.ceil(totalDeceasedMembers / limit);
...
export const adminUpdateDeceased = async (req, res) => {
  // const member = await Member.findById(req.params.id);
  const updatedDeceased = await DeceasedMember.findByIdAndUpdate(
    req.params.id,
    req.body,
  ...
  res
    .status(StatusCodes.OK)
    .json({ msg: 'deceased modified', deceasedMember: updatedDeceased });
};
