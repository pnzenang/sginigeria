...
export const validateIdParamForDeceased = withValidationErrors([
  param('id').custom(async (value, { req }) => {
    const isValidId = mongoose.Types.ObjectId.isValid(value);
    if (!isValidId) throw new BadRequestError('invalid mongoDb id');

    const deceasedMember = await DeceasedMember.findById(value);
    if (!deceasedMember)
      throw new NotFoundError(`no deceased member with id ${value}`);

    const isAdmin = req.user.role === 'admin';
    const isOwner = req.user.userId === deceasedMember.createdBy.toString();
...
export const validateRegisterInput = withValidationErrors([
  ...
  body('email')
    ...
    .custom(async (email) => {
      ...
      if (user) {
        throw new BadRequestError('email already exists, choose another email');
      }
    }),
...
export const validateDeadMemberInput = withValidationErrors([
  body('deceasedMemberMatriculation')
    .notEmpty()
    .withMessage('a member matriculation is required')
    .isLength({ min: 12, max: 12 })
    .withMessage('association code should have 12 characters')
    .custom(async (deceasedMemberMatriculation) => {
      const member = await DeceasedMember.findOne({
        deceasedMemberMatriculation,
      });
      if (member) {
        throw new BadRequestError(
          `This death was already announced, Thank you`
        );
      }
    }),
]);
